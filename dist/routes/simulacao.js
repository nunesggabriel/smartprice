...
router.get('/', async (req, res) => {
    try {
        const simulacoes = await prisma_1.default.simulacao.findMany({
...
router.get('/:id', async (req, res) => {
    ...
    try {
        const simulacao = await prisma_1.default.simulacao.findUnique({
            where: { id },
            include: {
                produto: { select: { id: true, referencia: true, descricao: true } },
            },
        ...
        if (!simulacao)
            return res.status(404).send({ message: 'Simulação não encontrada' });
...
router.post('/:produto_id', async (req, res) => {
    ...
    try {
        ...
        const despesas = await prisma_1.default.contasPagar.findMany({
            where: {
        ...
        const todosProdutos = await prisma_1.default.produto.findMany({
            select: { producao_mensal: true },
        });
        const totalProducao = todosProdutos.reduce((s, p) => s + (p.producao_mensal ?? 0), 0);
        ...
        const impostos = await prisma_1.default.imposto.findMany();
        const percentualImpostos = impostos.reduce((s, i) => s + Number(i.percentual ?? 0), 0);
        ...
        const simulacao = await prisma_1.default.simulacao.create({
            data: {
                produto_id,
...
router.delete('/:id', async (req, res) => {
    const id = Number(req.params.id);
    try {
        const existe = await prisma_1.default.simulacao.findUnique({ where: { id } });
        if (!existe)
            return res.status(404).send({ message: 'Simulação não encontrada' });
        await prisma_1.default.simulacao.delete({ where: { id } });
        res.status(200).send();
    }
    catch {
