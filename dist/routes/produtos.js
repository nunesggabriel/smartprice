...
router.post('/', async (req, res) => {
    ...
    try {
        const existe = await prisma_1.default.produto.findFirst({
...
router.put('/:id', async (req, res) => {
    const id = Number(req.params.id);
    try {
        const existe = await prisma_1.default.produto.findUnique({ where: { id } });
        if (!existe)
            return res.status(404).send({ message: 'Produto não encontrado' });
        const produto = await prisma_1.default.produto.update({ where: { id }, data: { ...req.body } });
        res.status(200).send(produto);
    }
    catch {
